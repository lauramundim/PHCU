...
object Main { 
  ...
  //cria arquivo de solução para gravar as respostas
   ...
   val PATAT2002 = List("medium01.tim", "medium02.tim", "medium03.tim", "medium04.tim",
   ...
   for(i<-0 until lerPar.par.length){
     ...
     for(j<-0 until instâncias.length){ 
       ...
       for(k<-0 until instâncias(j).length){
         ...
         if(k>=5 && k<=9) par.limiteExec =  900
         if(k>=10) par.limiteExec =  900
         println("------- o limite agora eh: " + par.limiteExec)
