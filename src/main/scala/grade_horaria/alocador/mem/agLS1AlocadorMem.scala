...
import grade_horaria.{AGBase, Parâmetros, MEM, 
                      SoluçãoGuiada, Solução}
import grade_horaria.alocador._

class AGLS1AlocadorMem(
  par:Parâmetros,
  arqProb: String,
  ...
  def run():População = {
    ...
    var mem = cmem.empty
    //println("------------!!!! gerações: " + par.nMaxGeração + " tempo " + par.limiteExec)
    ...
    while (geração <= par.nMaxGeração && 
      ...
      //println("AGORA: " + geração + " " + par.nMaxGeração + 
