package grade_horaria.basico.mem

import ag.Cronômetro
import grade_horaria.{AGBase, MEM, SoluçãoGuiada, Parâmetros, Solução}
...
class AGBasicoMem(
...
) extends AGBase[Indivíduo, População](par, arqProb){
  val cronômetro = new Cronômetro
  val movimentos = new MovimentoAleatório(prob)
  val mutação = new MutaçãoPop(par, movimentos)
  val torneio = new Torneio(par)
  val cruzamento = new Cruzamento(par, prob)
  ...
  def run():População = {
    cronômetro.iniciar
    var geração = 1
    var pop =  População(prob,fapt, par.tamPop)
    ...
    println*/
    if(arqSol != " ")
      sol.gravaCtt(prob, arqSol, pop.melhorIndivíduo.cromossomo)
    pop
  }
}
